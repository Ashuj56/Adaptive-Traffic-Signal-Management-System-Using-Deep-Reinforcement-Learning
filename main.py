...
class Config:
    ...
    FAIRNESS_LIMITS = {0: 8, 1: 7, 2: 8, 3: 7}  
...
class TrafficSignal:
    ...
    def activate(self, green_time: int):  
...
class Vehicle(pygame.sprite.Sprite):
    ...
    def _check_vehicle_ahead(self) -> bool:
        ...
        safe_distance = Config.MOVING_GAP + 3  
...
class SimulationManager:
    ...
    @classmethod
    def calculate_demand_score(cls, direction_idx: int) -> float:  
    ...
    @classmethod
    def _enhanced_signal_control_loop(cls): 
        ...
        while True:
            ...
            cls._enhanced_adaptive_selection()  
    ...
    @classmethod
    def _update_enhanced_timers(cls):  
    ...
    def _enhanced_adaptive_selection(cls):  
        """Select next signal using enhanced adaptive algorithm with weighted fairness."""
        cls.cycle_counter += 1
        
        # Check for fairness enforcement
        fairness_candidates = cls._get_fairness_candidates()
        adaptive_candidates = cls._get_adaptive_candidates()
        
        # Weighted selection: prefer adaptive but allow fairness
        if random.random() < Config.ADAPTIVE_WEIGHT and adaptive_candidates:
            selected_direction = max(adaptive_candidates, key=adaptive_candidates.get)
            selection_type = "ADAPTIVE"
            cls.results["adaptive_used"] += 1
        elif fairness_candidates:
            selected_direction = max(fairness_candidates, key=fairness_candidates.get)
            selection_type = "FAIRNESS"
            cls.results["fairness_used"] += 1
        else:
            # Fallback to round-robin
            selected_direction = (cls.current_green + 1) % Config.NUM_SIGNALS
            selection_type = "ROUND-ROBIN"
        
        # Calculate green time based on demand
        demand_score = cls.calculate_demand_score(selected_direction)
        green_time = max(Config.DEFAULT_MINIMUM, min(int(demand_score), Config.DEFAULT_MAXIMUM))
        ...
        cls.results["decision_log"].append({"cycle": cls.cycle_counter, "type": selection_type})
    ...
    def _generation_loop():
        ...
        while True:
            ...
            if SimulationManager._can_spawn_vehicles():
                ...
                will_turn = 0
                if lane_number == 2: 
                    will_turn = 1 if random.randint(0, 6) <= 2 else 0  
        ...
        for i in range(Config.NUM_SIGNALS):
            ...
            if i == SimulationManager.current_green:
                if SimulationManager.current_yellow == 1:
                    ...
                    signal.signal_text = str(signal.green) if signal.green > 0 else "WAIT"
            else:
...
class TrafficSimulationApp:
    ...
    async def _handle_keypress(self, key):  
        ...
        if key == pygame.K_ESCAPE:
            ...
            self._print_detailed_analytics()  
        elif key == pygame.K_p:
    ...
    def _print_detailed_analytics(self):  
